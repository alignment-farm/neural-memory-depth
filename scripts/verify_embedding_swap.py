...
def main():
    ...
    assert json.loads((out/'preflight.json').read_text())['source_definitions']==study.prior.prior.refresh.pilot.engine.loaded
    ...
    checks=[];origins={'new':0,'reused_embedding_baseline':0};donors=study.donors()
    for config in study.CONFIGS:
        ...
        for name,x in model.state_dict().items():
            ...
            seed=config[study.group(name)] if study.group(name) else 16
        ...
        for stream in study.SEEDS:
            ...
            assert r['origin']==('reused_embedding_baseline' if reused else 'new');origins[r['origin']]+=2
            if reused:
                ...
                for key,value in donor.items():
                    if key not in ['configuration','label','origin','donor_path','donor_sha256']:assert r[key]==value,key
    ...
    assert origins=={'new':120,'reused_embedding_baseline':40}
    ...
    print('Verified all 160 embedding-swap outcomes and unchanged baselines.')
