...
def main():
    ...
    for config in study.CONFIGS:
        ...
        for name,value in model.state_dict().items():
            source=config[study.group(name)] if study.group(name) else 16
            assert torch.equal(value,donors[source].state_dict()[name])
    for seed in [11,16]:
        config={g:seed for g in study.GROUPS}
        with contextlib.redirect_stdout(io.StringIO()):
            old=study.prior.train_pair({'memory':16,'embeddings':seed,'readout':16},99,data,10,True)
    ...
    report={'all_group_origins_verified':True,'pure_donor_training_checks':checks,
        'source_definitions':study.prior.prior.refresh.pilot.engine.loaded,'script_sha256':study.sha(__file__)}
