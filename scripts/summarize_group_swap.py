...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'analysis/group_swap';SEEDS=list(range(10,20))
GROUPS=['memory','embeddings','readout'];CONFIGS=[dict(zip(GROUPS,v)) for v in itertools.product([11,16],repeat=3)]
def label(c):return f"m{c['memory']}-e{c['embeddings']}-r{c['readout']}"
def main():
    ...
    for ax,mode in zip(axes,['released','complete']):
        ...
        ax.set_xticks(range(10),SEEDS);ax.set_yticks(range(8),[f"{c['memory']} / {c['embeddings']} / {c['readout']}" for c in CONFIGS])
        ax.set_xlabel('Training-stream seed');ax.set_ylabel('Donor: memory / embeddings / readout');ax.set_title(mode.title())
    ...
    fig.suptitle('Untrained parameter-group swaps: all eight donor combinations')
